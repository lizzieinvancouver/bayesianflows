...
\abstract{Improvements in algorithms and computational speed have heralded a new era of Bayesian model fitting. Models are easier to fit, faster to run, and more flexible, making them ideal for many of the complexities of ecological sampling designs. This has opened up Bayesian approaches to a new world of users, but often ecologists are not aware of some of the advantages now offered. Ecologists frequently use the `same-old' tricks, approaches, and work-arounds that served them well for frequentist methods, and are not taking advantage of some of the new possibilities on offer.
% WDP: Something like this - the end of the last sentence didn't make sense to me.
  Here we present a Bayesian workflow---centering on model and data simulation
% WDP: What is 'model simulation'? Comes up later but its meaning is unclear here
  ---that represents a ground shift of not just how to do fit Bayesian models, but how we should approach statistical model fitting to advance our science. This workflow integrates mechanistic and statistical models with a computational toolkit, which we argue would accelerate ecological science. Steps of this workflow will often highlight current limits of our inference and suggest paths forward, as they provide a clear method to test how uncertain models are given small sample sizes, or how often a poor model may appear `best' given certain diagnostics. By interrogating our methods through generative models and data from such models, we can refine where to put resources for better estimates, better models, and better forecasts. While we outline these methods from the happy surf of our Bayesian beach, the ocean we describe works for anyone fitting models to ecological data.
  % WDP: I like that ending, it sets a jolly tone and I think is likely to encourage people to read on as this could otherwise be quite dense...
...
% WDP: The 'jump from the jungle' bit is good, but I worry the anal marine person is going to say "I'm not a forest ecologist" and the whole edifice we're building comes crashing down...
...
% WDP: I feel like the natural history bit is a tangent here; perhaps delete or clarify?
...
% WDP: Careful, because Bayesians use p-values too (albeit of a different meaning) - I've reframed.
% [perhaps mention part of the appeal of these methods come from their ease of interpretation, a t-test provides a simple summary statistic to the question of whether two groups are different.]
...
% WDP: https://www.cell.com/trends/ecology-evolution/fulltext/S0169-5347(21)00284-6 but also isn't there a special issue in Ecology on all of this? Didn't Gelman have a paper in it?
...
% WDP: Should there be some kind of caveating sentence or two warning that what follows isn't very technical and isn't complete? A reference to Gelman for more details?...
...
% WDP: "Likelihood" is more technical than I would like to be in this section
...
% WDP: I think I agree with the above, and it feels like a straw-man to knock down
...
% WDP: MCMC is it, surely? See my summary above.
...
% WDP: They're not half of the equation; they're the smallest part if you have a reasonable amount of data (see all my ranting)
