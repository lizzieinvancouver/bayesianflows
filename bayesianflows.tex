...
\emph{In a nutshell:} 
\begin{itemize}
\item Climate change, habitat degradation, biodiversity loss and other human-caused pressures on ecological systems have increased the need for accurate forecasts that can help managers and policy-makers mitigate negative impacts. 
\item At the same time, larger ecological datasets provide a major opportunity for improved models that underlie better forecasts, but building the most useful models that can handle these larger, messier datasets generally requires skills most ecologists were not trained in.
\item Here, we outline a workflow of how to approach this challenge. It builds on new insights from statistics and data science for how to better integrate simulating data at multiple points to gain insights into how systems work and where forecasts may be most useful. While we outline this workflow using a Bayesian approach, it can be applied to many methods. 
\item We conclude by reviewing how this workflow could improve training in ecology, making the steps from model development to forecasts more tractable for diverse practitioners in ecology and conservation biology. 
\end{itemizer}
