...
\abstract{Ecology is a discipline facing increasing challenges given growing demands for robust predictive models. Meeting this demand requires approaches that can handle bigger data to yield forecasts that capture the variability and underlying uncertainty of ecological systems. Yet many ecologists are not trained in current methods to build flexible robust models, and thus often rely on a limited set of pre-defined models combined with null hypothesis testing or take up new approaches without sufficient background to understand their benefits and pitfalls. The result is poor models that lead to incorrect predictions and decisions, alongside concerns of a looming replication crisis---a result not unique to ecology. 
...
Here we describe a broadly generalizable workflow for statistical analyses and show how it can enhance training in ecology, with potential extensions to other fields. 
...
Building on the increasingly computational toolkit of many ecologists, this approach leverages simulation to integrate model building and testing for empirical data more fully with ecological theory. It leverages new advances in workflows from statistics, that have yet to be widely used in ecology. We argue that greater use of this workflow can help fit models that are more robust and well-suited to provide new ecological insights---allowing us to refine where to put resources for better estimates, better models, and better forecasts.}
