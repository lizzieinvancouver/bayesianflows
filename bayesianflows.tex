...
% inference -- estimating some behavior from the data, 
% calibration -- analyzing how often an estimate is close to the true value.
...
Statistical analyses are designed for inference---to estimate some process or effect or behavior from data. Inference relies on calibration, however, which tells how often an estimate is close
to the true underlying value. Most ecologists are trained primarily in frequentist methods, which are focused on calibration (this is is why training in frequentist methods includes so much discussion of significance, power, mean squared error, and the like). Bayesian methods typically focus on inference, but can be equally well calibrated (see Step 1, below). A major problem with frequentist approaches in ecology today is that they can lead to more unpredictable inferential behaviors, but ecologists are not trained how to recognize or deal with this. Our workflow provides a set of useful organized steps to explicitly study those potential inferential behaviors and prepare for their consequences. We believe it's much easier and straightforward to do with a Bayesian approach and thus outline the workflow with that approach in mind. 
