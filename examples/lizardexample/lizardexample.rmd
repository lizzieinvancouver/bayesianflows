...
Here we imagine a very simple example where you are interested in whether a storm that hit several tropical islands, destroying vegetation on the islands, may have led lizards to evolve shorter toes -- since they may longer need them to run up vegetation. If you plan to sample one lizard (they're hard to catch and you have limited time) on each of the five islands and one five islands that were not by a storm. You could build a very simple linear regression model to simulate these data:
...
Your model is returning your parameters pretty well, but this is only one draw of all the possible ones you might get. You can repeat this process 1000 times, however: 
...
What would happen if you somehow could sample more and get 10 reps of each? You can easily test this. 
...
Now only about 1% of time you see no effect or a positive effect. You can equally adjust other parameters to see their effect on returning your parameter estimates, or see how often your p-value is significant if that is of interest. You might also start considering other experimental designs such as sampling multiple lizards per island and adjust your simulated data and model to reflect this. This might get you to thinking about the evolution of toe length and lead you to model how that happens, which might in turn alter your simulated data and statistical model -- and it would mean you've gone from thinking about the problem as a linear regression to something closer to a biological model. 