...
Next we want to consider our priors. Here we show one way to visulize what the priors allow, looking at 12 plots where we drew different possible hyperparameters for the intercept and slope from the priors:
```{r}
...
In this case, we had a hypothesis that the partial pooling on the intercepts was perhaps not an accurate model. Our data spanned a mix of biomes (marine, terrestrial, aquatic) and events (pollination, predation) and we were not sure these should actually be normally distributed. We decided to remove the partial pooliing on the intercepts. We then repeated steps 2-3 and looked again at the retrodictive checks (we show only the final one here):
...
This model showed a lower standard deviation (and a far lumpier one), though it was still well away from the empirical data. What could be going on? We had hypotheses about the species, biomes and event types, we went on to test in our research program. 