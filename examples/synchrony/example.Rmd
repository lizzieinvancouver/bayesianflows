...
for (sp in 1:Nspp)
  lines(year[species==sp], y[species==sp], col="hotpink3")
```
...
plot(spslopes~species_trend, xlab="Given species-level slopes", ylab="Modeled species-level slopes", col="hotpink3")
abline(0,1)
```
...
```{r}
par(mfrow=c(1,2))
hist(rnorm(5000, 100,30), main="Intercept mean prior", col="lightpink")
segments(91,25,213,25, lwd=5, col="hotpink4") # April 1 to August 1
hist(rnorm(5000, 0, 5), main="Slope (days per year) mean prior", col="lightpink")
segments(-10,25,0,25, lwd=5, col="hotpink4")
```
...
hist(yreal, xlab="Day of year", main="Empirical data")
...
hist(y, xlab="Day of year", col="mediumpurple3", main="Data from posterior means")
```
...
par(mfrow=c(1,1))
hist(colMeans(y.sd100), col="mediumpurple1", breaks=20, xlim=c(10,14), 
    main="",
    xlab="Mean SD of response from 1000 sim. datasets (light purple) \n versus empirical data (dark blue line)")
...
This model showed a slightly lower standard deviation, though it was still well away from the empirical data. What could be going on? We had hypotheses about the species, biomes and event types, we went on to test in our research program. 