...
% Fix Example 2 (last comment of R1) and sneak in spatial correlation to Example 3
...
Good point. We overhauled the introduction to more broadly position ecology as one of many fields that has shifted in its aims and resulting study design from simple approaches focused on single study systems to more global datasets and aims (edits throughout \lr{newintrostart}-\lr{newintroend}).\\
...
Building on the increasingly computational toolkit of many ecologists, this approach leverages simulation to integrate model building and testing of empirical data more fully with ecological theory.
\end{quote}
...
\end{quote}
But we can see now that we overstate this in other places in the manuscript. To address this, we have cut and edited the text suggesting how broadly we describe the workflow (\lr{beyondMCMC2}, \lr{beyondMCMC3}) and linked more clearly to examples using other methods (e.g., \lr{beyondMCMC4}). We also made small changes throughout (e.g., \lr{bayesfocus}, \lr{mobe}) to clarify this. \\
...
This is a great point and we very much agree. We added several sentences to this paragraph to make these points and added the suggested reference (\lr{betterppcstart}-\lr{betterppcend}).\\
...
Done, though we used 366 to include leap years (\lr{calendarday}).\\
...
Thanks for catching this. We changed to (\lr{huh}): ``The increasingly computational toolkit of the modern ecologist makes it easier to bridge the gap between statistical models used in ecology and the field's underlying mechanistic theories."\\
...
We have adjusted the text in this workflow to address these concerns.\\
...
This is a great point as we think the workflow is useful for all analysis, not just the complex ones, but we did not explain this well enough before. To address this we have revamped much of the introduction, see 
...
We have worked to add this point, see \lr{rrinference}. \\ % lr{Rnewmodels}-
