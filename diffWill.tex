...
%DIF DEL bayesianflows.tex      Fri Feb  2 11:36:34 2024
%DIF ADD bayesianflowsWDP.tex   Sat Feb  3 10:50:33 2024
...
% 160 words (200 max for NEE); main text is 4,000; 3-4K for a perspective, see https://www.nature.com/natecolevol/content % Currently >7,000 words
...
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \abstract{Growing anthropogenic pressures have increased the need for robust predictive models from ecologists.
% WDP: The abstract is good, and I like the theoretical vs. empirical spin. If you are going for NEE, I think the opening is a bit dry. Something like "The growing number of threats to biodiversity and human well-being mean ecologists are increasingly called upon to forecast future scenarios and not just model past events."
  To meet this demand, ecologists need approaches that can handle bigger data and yield forecasts that capture the variability of ecological systems, while also including the underlying uncertainty needed to communicate the inherent risks and trade-offs in decision-making. Bayesian models are especially adept at this and are growing in use in ecology. Yet most ecologists today, whether using Bayesian or frequentist approaches, are not trained in take advantage of bigger ecological data and more flexible robust models. We argue this stems from an antiquated divide in training for `theoretical' versus `empirical' ecologists that we can shift with new practices. Here we describe a broadly generalizable workflow for statistical analysis centered on simulating data from models, and show how it can revolutionize training in ecology. Building on the increasingly computational toolkit of many ecologists, this approach represents a ground shift of not just how to build Bayesian---or frequentist---models, but how we should approach model fitting to advance our science.  By integrating model building and testing more fully with ecological theory,  this simulation-based workflow helps fit models that are more robust and well-suited to provide new ecological insights. This in turn can help us refine where to put resources for better estimates, better models, and better forecasts.}

 \DIFaddend % Steps of this workflow will often highlight current limits of our inference and suggest paths forward, as they provide a clear method to test how uncertain models are given small sample sizes, or how often a poor model may appear `best' given certain diagnostics. 
...
\DIFdelbegin \section*{\DIFdel{Main text}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Introduction}}
\DIFaddend Recent years have seen an explosion \DIFdelbegin \DIFdel{in the use }\DIFdelend of Bayesian models across scientific fields \citep{vandeschoot2021,schad2021,grinsztajn2021}, including ecology. This \DIFaddbegin \DIFadd{change }\DIFaddend comes in part from increased computational power, but mostly from new algorithms \citep[e.g. Hamiltonian Monte Carlo,][]{nuts2014,betan2019} that have made fitting and implementing Bayesian models faster, more robust and---in many ways---easier \citep{Carpenter:2017stan}. \DIFdelbegin \DIFdel{Capitalizing on these advances }\textsf{\DIFdel{R}} %DIFAUXCMD
\DIFdel{packages, such as }\textsf{\DIFdel{brms}} %DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\citep{burkner2017brms}}\hskip0pt%DIFAUXCMD
, that streamline fitting a set of pre-determined models have seen growing use. Bayesian approaches, long heralded as more powerful, flexible and especially adept at capturing the multiple levels of variance in ecological data, are positioned to advance ecology as a discipline. %DIF <  They could also steer it away from null hypothesis testing, and its related focus on $p$-values, which have led to replication crises in other fields. % Bayesian approaches, long heralded as more powerful, flexible and especially adept at capturing the multiple levels of variance in ecological data, seem poised to advance ecology as a discipline, and to avoid contributing to the replication crisis.
}%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Capitalizing on these advances \textsf{R} packages, such as \textsf{brms}, have seen much wider use than the previous generation of packages that streamlined fitting a set of pre-determined models using Bayesian approaches (e.g. \textsf{MCMCglmm}). We believe that Bayesian approaches, long heralded as more powerful, flexible and especially adept at capturing the multiple levels of variance in ecological data, are positioned to advance ecology as a discipline.
 %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{At the same time, ecology itself appears well positioned to apply a century of theory and empirical insights to address the global challenges society is facing. Growing anthropogenic pressures have increased the need for predictive models from ecologists . Policy-makers demand }\DIFdelend \DIFaddbegin \DIFadd{The flexibility of Bayesian methods have been put to quick use by ecologists to provide }\DIFaddend forecasts of how \DIFdelbegin \DIFdel{communities and ecosystems will shift with climate change, habitat degradation, and other anthropogenic forces tobalance conflicting societal needs and desires. The solutions ecologists can offer are rarely simple, and communicating the inherent risks and trade-offs in decision-making would benefit from forecasts that include uncertainty, and the variability of ecological systems}\DIFdelend \DIFaddbegin \DIFadd{biodiversity can provide, and respond to, the ever-shifting needs and desires of society}\DIFaddend .

%DIF <  To meet these demands, ecology has developed ever larger datasets to test fundamental theory across systems \citep{Hampton2013}, but bigger data is messier data and ecological training rarely includes the skills needed to generate robust forecasting and inference from such datasets. 
\DIFdelbegin \DIFdel{To meet these demands, ecology has }\DIFdelend \DIFaddbegin \DIFadd{As ecologists grapple with these great societal questions, they have }\DIFaddend developed ever larger datasets to test fundamental theory across systems \citep{Hampton2013}, but bigger data is messier data \DIFaddbegin \DIFadd{that ecologists are  not usually trained to handle}\DIFaddend . Such data generally requires a model of both the underlying biological processes and how the measurements were made. While some fields have long used these types of models \citep[generally in fields focused on inferring population sizes of things people want to eat or manage,][]{muthuku2008,zheng2007,trijoulet2018,strinella2020potential}, most have not. Thus \DIFdelbegin \DIFdel{many }\DIFdelend \DIFaddbegin \DIFadd{ecologists }\DIFaddend have tried to adapt what they were trained in---traditional statistical methods (e.g., $F$ and $t$ tests) and a strong focus on null hypothesis testing. Often they have done this by fitting multi-way interaction terms (rather than using mechanistic models informed by biological understanding), using random effects to correct for group level factors (rather than explicit models of data-collection biases), or comparing across a large suite of models (because they cannot fit a single model that accounts for all the idiosyncrasies and biases).
%DIF <  and so aim to average out the noise to leave only the signal 

\DIFdelbegin \DIFdel{But these approaches do not often align with ecology's aims today. Beyond the reality that most traditional methods are fragile when used beyond the cleaner, simpler experiments these methods assume (e.g. spatial, temporal and phylogenetic correlations often violate these methods independence assumptions), they will usually fail to produce robust, reproducible results. Multi-way interaction terms can make main effects hard to interpret and require much larger sample sizes to estimate reliably. Null hypotheses are rarely true and can lead to confusion over what is important versus `significant' \mbox{%DIFAUXCMD
\citep{gelmanhill,muff2022rewriting}}\hskip0pt%DIFAUXCMD
. Many model comparison and machine learning approaches prefer models whose inferences match the idiosyncratic fluctuations in the ecological data to which they are fit, but don’t generalize to other observations. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The disconnect between the traditional statistical tools available to the ecologist and the fundamental theory they are trying to test is being made ever-clearer as ecologists are called upon to make forecasts. }\DIFaddend Bayesian approaches provide a pathway to powerful models that can transform how we understand our systems \DIFdelbegin \DIFdel{as large-scale ecological data increasingly becomes available for more diverse systems and new questions}\DIFdelend \DIFaddbegin \DIFadd{in terms of fundamental theory}\DIFaddend . Ecologists recognize this potential and are increasingly using Bayesian approaches, but are often inadequately prepared to notice or manage the pitfalls of such models. Many of these pitfalls can be avoided by approaching analyses through specific workflows \citep{betanworkflow,grinsztajn2021,vandeschoot2021}, which themselves are built on a process of how to do not just statistics, but how to do science \citep{box1976science}. % Maybe address workflow comment here? 

These approaches move away from a focus on null hypothesis testing, towards estimating effect sizes, using models calibrated (see Table \ref{tab:glossary}) and better understood through simulating data at multiple \DIFdelbegin \DIFdel{steps---using }\DIFdelend \DIFaddbegin \DIFadd{steps---
%DIF >  WDP: The bits from "using models calibrated" aren't clear to me - maybe can you rephrase?
using }\DIFaddend a number of skills often reserved in ecology more for `theorists' than empirical ecologists.
%DIF >  WDP: Here I think we're missing that nice link with fundamental theory, which I think will bring the 'previous generation' of scientists along with us for the ride here
Given the societal demands on ecologists today, we argue this divide is \DIFdelbegin \DIFdel{antiquated and limits progress. It reserves the insights gained into how ecological systems work, which come from building models of them, for a select group. Further, it ignores that the }\DIFdelend \DIFaddbegin \DIFadd{antiquated---especially as the }\DIFaddend average modern ecologist is computational\DIFdelbegin \DIFdel{; thus many already have }\DIFdelend \DIFaddbegin \DIFadd{, we believe they also need }\DIFaddend the skills to \DIFdelbegin \DIFdel{bridge the }\DIFdelend \DIFaddbegin \DIFadd{build and understand their own models. Breaking the training }\DIFaddend divide between theoretical and empirical ecologists\DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{, we argue, would yield greatly increased flexibility in what models can be fit and improved insights into the underlying systems.
}\DIFaddend % They move away from a focus on null hypothesis testing, towards estimating effect sizes, using models calibrated (see Table \ref{tab:glossary}) and better understood through simulating data at multiple steps---allowing greatly increased flexibility in what models can be fit and improved insights into the underlying systems. 
...
\DIFdelbegin \subsection*{\DIFdel{A short guide to statistical workflows and Bayesian approaches}}
%DIFAUXCMD
\DIFdel{Statistical analyses are designed for inference---to learn about some process or effect or behavior from data  (see Table \ref{tab:glossary}). Robust analyses, however, rely on our inferences being consistent with the underlying truth more often than not.  Quantifying this consistency is calibration---analyzing how often a parameter estimate is close to the true value---a critical part of using models for inference. A major problem with traditional (frequentist) approaches in ecology today is their inferences are unpredictable when their foundational assumptions are not met, but ecologists are not usually trained in how to recognize or deal with this.
%DIF <  WDP: This bit comes across as weaker than intended (to my eyes, which are a bit biased because I get a bit antsy about axiomatically assuming anything in stats) because it almost (but doesn't quite, I recognise) imply that only frequentists assume something, and I think it might get people's backs up a bit. Perhaps, instead, you could lean away from the Bayes vs. frequency bit here and point out that parts of this approach are agnostic with regards to frequency vs. Bayes (that makes it sounds like we're saying something very new) but that to get the full advantages of it you have to use Bayes. Lean into the "learn to calibrate and so infer better" angle more strongly? 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The workflow we recommend helps avoid these problems. It provides an organized sequence of steps to explicitly build and challenge a model through simulation. We believe this is easier and more straightforward to do with a Bayesian approach and thus outline the workflow assuming a Bayesian approach, but we suggest a similar approach could, and should, be extended to other statistical inference methods. %DIF <  Most ecologists are trained primarily in frequentist methods, which are focused on calibration (this is is why training in frequentist methods includes so much discussion of significance, power, mean squared error, and the like). Bayesian methods typically focus on inference, but can be equally well calibrated (see Step 1, below). 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section{\DIFadd{A brief overview of Bayesian methods and their advantages}}
\DIFaddend To better understand our workflow, we provide a very brief overview of some of the fundamentals of Bayesian methods that is inherently incomplete and, by design, not very technical. This section can be skipped for those who feel already well-versed, and can be augmented for those who are new to Bayesian approaches \citep[for example,][]{statrethink,BDA,regotherstories}.

\DIFdelbegin \emph{\DIFdel{A brief review of statistical inference using Bayesian approaches}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Bayesian and frequentist probability and inference}}
\DIFaddend 

Probability is often defined as ``the long-term frequency with which something happens.'' \DIFdelbegin \DIFdel{We would expect, for example that if we tossed a coin }\DIFdelend \DIFaddbegin \DIFadd{This is the frequentist definition, and it's also an intuitive definition, and it is excellent for checking whether a given observation (data, such as ``45 coins tossed out of }\DIFaddend 100 \DIFdelbegin \DIFdel{times we would see roughly }\DIFdelend \DIFaddbegin \DIFadd{came up heads'') is consistent with a given model (such as ``}\DIFaddend 50\DIFdelbegin \DIFdel{heads.  In this case we would say that the probability of tossing a coin and getting a `head' is $\frac{50}{100}$, equivalent to 50\% or $\frac{1}{2}$. At the same time we wouldn’t be very surprised if we observed 49 or even 55 heads, although we would be surprised if we saw 99.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This definition of probability---which is the }\emph{\DIFdel{frequentist}} %DIFAUXCMD
\DIFdel{definition---is useful in many situations, but ithas a few disadvantages. First, frequentist definitions aren't very helpful when dealing with unexpected situations. Frequentist probabilities are grounded in repeatable observations, and so understanding these repeatable frequencies is of limited use when trying to make predictions for changing or entirely novel systems. Second, most common frequentist approaches rely on specific assumptions. Using frequentist statistics requires trying to match a model that we have (often just in our heads) of some ecological system to a frequentist method that mostly closely matches the assumptions of our biological model. Given the complexity of ecological data and our uncertainty about the underlying model , frequentist approaches can be especially challenging in ecology. }\DIFdelend \DIFaddbegin \DIFadd{\% of tossed coins come up heads''). This excellence of probability of }\emph{\DIFadd{data}} \DIFadd{given }\emph{\DIFadd{model}} \DIFadd{($p(data|model)$) leads to frequentist statistics focusing on }\emph{\DIFadd{refuting}} \DIFadd{whether a given set of observation is consistent with a particular null model, but makes it challenging to estimate the probability of a particular model ($p(model|data)$). Put another way, a frequentist test excells at telling us the probability that a given model is wrong (``I reject the null hypothesis that this is a fair coin''), but cannot tell us the probability that the coin is biased (``there is a 95\% probability that this coin comes up heads at least 80\% of the time'').
}\DIFaddend 

\DIFdelbegin \DIFdel{Bayesian approaches provide a wayto build models that can propagate our (un)certainties about what we do---or especially, don't---know about how ecological systems work. In the Bayesian view, $probability$ is used to quantify uncertainty: the higher }\DIFdelend \DIFaddbegin \DIFadd{Bayesian methods can estimate }\DIFaddend the probability of a \DIFdelbegin \DIFdel{certain interval of values the less uncertainty we have that the true behavior falls within that interval. Assuming we also have some estimate of our initial uncertainty---usually from knowledge of ecology---to inform a prior distribution (termed below $prior$), then we can apply Bayes’ }\DIFdelend \DIFaddbegin \DIFadd{model, and that remarkable feat alone is enough to confer a major advantage when confronting a policy-maker (``what is the probability that those trees will die if I apply this herbicide''). This is done through the application of Bayes' }\DIFaddend theorem: 

\begin{equation}
  probability = \DIFaddbegin \DIFadd{p(model|data) = }\DIFaddend \frac{likelihood \cdot prior}{normaliser} \DIFaddbegin \DIFadd{= \frac{p(data|model) \cdot p(model)}{p(data)}
  }\DIFaddend \label{bayes_theorem}
\end{equation}

\DIFdelbegin \DIFdel{to update that prior into a posterior distribution that accounts for the information added by a likelihood ($likelihood$ is the same as a frequentist likelihood). Here, }\texttt{\DIFdel{normaliser}} %DIFAUXCMD
\DIFdel{is a }\DIFdelend \DIFaddbegin \DIFadd{This equation, and its dependence on the }\emph{\DIFadd{normaliser}} \DIFadd{(a }\DIFaddend mathematical constant that makes sure our probability cannot go above 100\% or below 0\%\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend statisticians are lazy, and will not `give 110\%')\DIFdelbegin \DIFdel{. This mathematical constant }%DIFDELCMD < [%%%
\DIFdel{technically it is the probability of our data; $p(data)$}%DIFDELCMD < ] %%%
\DIFdel{is a nuisance term that is extremely challenging to estimate (sometimes it is impossible!) and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend held back the practical \DIFdelbegin \DIFdel{application }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend of Bayesian statistics for \DIFdelbegin \DIFdel{many years. Nowadays, with increases in computer power, we can use numerical methods such as `}\DIFdelend \DIFaddbegin \DIFadd{almost a century because that normaliser could rarely be analytically worked out. But one of the major advantages of Bayesian methods is that the solution to this problem---numerical simulations based on }\DIFaddend Markov Chain Monte Carlo \DIFdelbegin \DIFdel{' }\DIFdelend (MCMC)\DIFdelbegin \DIFdel{methods to avoid having to estimate its value precisely. Such methods are iterative algorithms  that involve chance at each step (that the process proceeds in iterations or steps, and only the last iteration affects the next, is why this is a `Markov Chain'), tweaking and changing parameters within your model until a distribution of parameters consistent with the data are found. This distribution is called the `posterior' distribution to distinguish how it is our view of probability }\emph{\DIFdel{after}} %DIFAUXCMD
\DIFdel{the data(via the likelihood) and the prior have been considered.
%DIF < There is no guarantee that the posterior distribution will, itself, be found: the user must check for so-called `convergence' onto the true posterior distribution as the process involves randomness. It is for this randomness that `Monte Carlo' is added into the name, after a city famous for gambling.
}\DIFdelend \DIFaddbegin \DIFadd{---provide a huge additional advantage. Now that no analytical solution needs be found, any model that can be written out mathematically can be fit to data, giving the scientist more freedom of model structure.
}\DIFaddend 

%DIF <  It turns out there is a further advantage that such an approach gives us. While frequentist probabilities are all about the probabilities of observing data ($p(data|model)$: what are the chances of getting 99 heads in 100 coin-tosses?), Bayesian probabilities are all about the probabilities of observing models ($p(model|data)$: what are the chances this coin is biased towards heads?). This makes it straightforward to report the results of Bayesian analyses in terms of estimates of uncertainty and precision of coefficients (contrast ``I'm 80\% certain that coin comes up heads at least two-thirds of the time'' with ``I'm less than 5\% certain those last twenty coin tosses came from a fair coin'').
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  WDP: I started trying to write about how you can do conditional probabilities using posterior distributions but I realised that I'm beginning to stray into the benefits and pitfalls section below. 
%DIFDELCMD < 

%DIFDELCMD < %%%
\emph{\DIFdel{A brief overview of the benefits---and pitfalls---of Bayesian models}} 
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Bayesian models have many benefits, but an often-mentioned one is that `}\DIFdelend \DIFaddbegin \subsection{\DIFadd{Flexibility}} 
\DIFadd{The oft-stated claim that ``}\DIFaddend you can fit any model you want\DIFdelbegin \DIFdel{.' While this }\DIFdelend \DIFaddbegin \DIFadd{'' with Bayesian methods }\DIFaddend is not entirely true \citep{BDA,reid2019}, \DIFdelbegin \DIFdel{Bayesian modeling }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend options can feel limitless \DIFdelbegin \DIFdel{when compared to the models ecologists can fit }\DIFdelend \DIFaddbegin \DIFadd{compared to options available }\DIFaddend in popular modeling packages (\DIFdelbegin \DIFdel{e.g. }\DIFdelend \DIFaddbegin \emph{\DIFadd{e.g.}}\DIFadd{, }\DIFaddend \textsf{lme4}). As long as you can write out the likelihood of your desired model \citep[and sometimes even if you can't;][]{Sunnaaker2013} and assign priors to all parameters, you can generally \DIFdelbegin \DIFdel{`fit ' the model}\DIFdelend \DIFaddbegin \DIFadd{fit it}\DIFaddend . This includes non-linear \DIFdelbegin \DIFdel{ones}\DIFdelend \DIFaddbegin \DIFadd{models}\DIFaddend , non-Gaussian families (e.g.\DIFaddbegin \DIFadd{, }\DIFaddend Poisson, beta, or combinations thereof, such as hurdle models), hierarchical designs\DIFdelbegin \DIFdel{and any combination of these, as well as }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend `joint' models where parameters estimated in one equation appear in another \DIFdelbegin \DIFdel{, propagating uncertainty}\DIFdelend \DIFaddbegin \DIFadd{propagating uncertainty, and many combinations of the above}\DIFaddend . Such flexibility is incredibly powerful in ecology where data are often influenced by complex spatial or temporal patterns, non-linear processes are widespread, and common data types are non-Gaussian (e.g. counts \DIFdelbegin \DIFdel{, percent cover, etc.}\DIFdelend \DIFaddbegin \DIFadd{and percentage cover}\DIFaddend ). 
...
\DIFdelbegin \DIFdel{Researchers can estimate comparable effect sizes from $z$-scored data (in units of standard deviation), alongside estimates in meaningful natural units, such as per }%DIFDELCMD < \degree %%%
\DIFdel{C of warming or per hectare of habitat lost.
}\DIFdelend %DIF >  Researchers can estimate comparable effect sizes from $z$-scored data (in units of standard deviation), alongside estimates in meaningful natural units, such as per \degree C of warming or per hectare of habitat lost.
%DIF >  WDP: Removed the above because you can do that in frequentist models too

\DIFaddbegin \subsection{\DIFadd{Interpretability}}
\DIFaddend % WDP: Below is a point that I think follows from what you have above; it's also something I'm keen on, as I think I've banged on about by now. Apologies because this text is a little rough; hopefully the spirit is coming through though.
...
\DIFaddbegin \fi
%DIF >  WDP: Again, Removed the above because I think we need the bridging section and we're over space. I think the 'not unique to Bayes' is perhaps worth pulling into the section above, but in a place that would be appropriate (or with the bit where you say "look this workflow could work for frequentist methods too")
\DIFaddend 

\DIFdelbegin \subsection*{\DIFdel{A basic Bayesian workflow}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{A basic Bayesian workflow}}
\DIFadd{Statistical analyses are designed for inference---to learn about some process or effect or behavior from data  (see Table \ref{tab:glossary}). Robust analyses, however, rely on our inferences being consistent with the underlying truth more often than not.  Quantifying this consistency is calibration---analyzing how often a parameter estimate is close to the true value---a critical part of using models for inference. A major problem with traditional (frequentist) approaches in ecology today is their inferences are unpredictable when their foundational assumptions are not met, but ecologists are not usually trained in how to recognize or deal with this.
}

\DIFadd{The workflow we recommend helps avoid these problems. It provides an organized sequence of steps to explicitly build and challenge a model through simulation. We believe this is easier and more straightforward to do with a Bayesian approach and thus outline the workflow assuming a Bayesian approach, but we suggest a similar approach could, and should, be extended to other statistical inference methods.
}

\DIFaddend % WDP: See my major comments. Broadly I think this is good, but I think the steps need to highlight why they're *exciting* and *solve a problem*. The calibration one just needs a bit more explicit linking to the text above, and as I look now is essentially almost there anyway. 
...
\DIFaddbegin \iffalse
\DIFaddend Our workflow is explained mostly program-agnostically. Though at times we assume a user of \textsf{Stan}, a relatively new probabalistic programming language, that interfaces with \textsf{R, Python, Julia} (and more) to write bespoke Bayesian models \citep{Carpenter:2017stan}. We focus on \textsf{Stan} as its MCMC algorithm (a variant of Hamiltonian Monte Carlo, HMC) is \DIFdelbegin \DIFdel{fast }\DIFdelend \DIFaddbegin \DIFadd{well-used, fast, }\DIFaddend and produces specific output to warn of model fit issues
\DIFdelbegin \DIFdel{(i.e., divergent transitions) in a way other MCMC algorithms do not (e.g. Metropolis-Hastings or Gibbs), }\DIFdelend %DIF >  WDP: divergent transitions are not a feature of other MCMC methods so the statement I've deleted, combined with the "i.e.", comes across as misleading.
\DIFaddbegin \DIFadd{, }\DIFaddend but the basic workflow should apply to diverse implementations of Bayesian modeling, and can be extended to other approaches (frequentist, resampling, etc.). \\ % to write bespoke Bayesian models and underpins the \textsf{R} packages \textsf{brms} and \textsf{rstanarm}, which fit a suite of specific (pre-defined) models
\DIFaddbegin \fi
%DIF >  WDP: I re-read this bit and, given we're over space, I feel like we don't need to write it. I think we're name-dropping Julia to sound cool and, unless there is a strategic reason to do that, let's not do it
\DIFaddend 
...
\DIFdelbegin \emph{\DIFdel{Step 4: Check your model on data simulated from your empirical model output (also known as, posterior retrodictive checks)}} 
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Step 4: Check your model on data simulated from your empirical model output (also known as, posterior retrodictive checks)}} 
\DIFaddend 
...
\DIFdelbegin \emph{\DIFdel{Feedbacks \& workflows}}%DIFAUXCMD
...
\DIFdelbegin \subsection*{\DIFdel{How this workflow changed our science}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{How this workflow changed our science}} \DIFaddend % Surprising things that happened to us that may happen to you if you follow this workflow
...
\DIFdelbegin \emph{\DIFdel{Understanding nonidentifiability}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Understanding nonidentifiability}} \DIFaddend \\
...
\DIFdelbegin \emph{\DIFdel{Know your limits}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Know your limits}} \DIFaddend \\ % Dive into the simplified complexity conundrum
...
\DIFdelbegin \emph{\DIFdel{Looking at parameters, not p-values}} %DIFAUXCMD
...
This focus on the value of parameters scales up through this and other modeling workflows. Having a better sense of parameter values across different biological contexts, model parameterizations, and time periods gives a better sense of how the biological world works, including what's reasonable, possible or wildly unrealistic. Our focus on parameters has revealed estimates where plants start growing at -10\degree C and frogs advance their spring events $>$40 days per decade with warming---both of which led us to new insights about our models and data that advanced our scientific understanding, and which we would have missed with more traditional approaches.  % Suddenly, weakly informative priors make ever more sense .... 

\DIFdelbegin \subsection*{\DIFdel{How this workflow intersects with ecological training}} %DIFAUXCMD
%DIF < Challenges of the workflow compared to how we traditionally train ecologists
\DIFdelend \DIFaddbegin \section{\DIFadd{How this workflow intersects with ecological training}}
%DIF >  WDP: I think what this section is missing is a conceptual diagram that shows how you might approach a problem as a 'traditional' undergrad ecologist (e.g., fitting a straight line through something), a 'traditional' PhD ecologist (e.g., fitting a curve line that has an aysmptote), and then an 'our workflow' ecologist (e.g., fitting an explicit mechanistic relationship). Then a lot of the 'merging theory with empirical data' stuff will shine through.
\DIFaddend 
...
%DIF >  WDP: This reads a little oddly to me; they were never 'required' per se (I refuse to believe that no co-author on this has fit a WinBugs/OpenBugs/JAGS model with non-conjugate priors, if only in error).... 'Antiquated' is also an odd phrase: the need for them stems from mathematical proof; is the Pythagorean theorem antiquated? My tongue's in my cheek but you get my point?
Prior predictive checks provide a far more powerful way to understand how priors work within a particular model, and are more useful than rules about which priors should be fit in certain cases or memorizing which priors are conjugate \citep{betanprior}. % (and thus sometimes referred to as  `the crystal deodorant of Bayesian statistics')
...
%DIF >  WDP: We've already hit on these notes earlier. If we're looking for places to cut, maybe cut it earlier in the MS as these are a good three points here. And, to MB's point above: Bayes factors are (or were, perhaps, I could check) used in Bayesian phylogenetics so I would be surprised if they made it into ecology because they were already well known in evolutionary biology ~fifteen years ago.
