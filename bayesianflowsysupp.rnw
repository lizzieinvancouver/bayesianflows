...
\section*{Next steps}
 Our aim is to provide an approachable rubric for those new to fitting complex models, and is not intended to be a comprehensive overview...
 
 I wonder if it would be helpful to end with a short “Next Steps” section/paragraph, not more than a few sentences, emphasizing that this manuscript is only presenting the basics with some recommendations for longer/more-in-depth references that would be useful for interested readers to consider.  Definitely a soft requirement and I have no problems with submission as is!
...
\pagebreak
For our example workflow we use \textsf{Stan} and \textsf{R}, presented in Markdown (\verb|example.md|).
