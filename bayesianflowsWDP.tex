...
\abstract{Growing anthropogenic pressures have increased the need for robust predictive models from ecologists.
...
Recent years have seen an explosion of Bayesian models across scientific fields \citep{vandeschoot2021,schad2021,grinsztajn2021}, including ecology. This change comes in part from increased computational power, but mostly from new algorithms \citep[e.g. Hamiltonian Monte Carlo,][]{nuts2014,betan2019} that have made fitting and implementing Bayesian models faster, more robust and---in many ways---easier \citep{Carpenter:2017stan}. The flexibility of Bayesian methods have been put to quick use by ecologists to provide forecasts of how biodiversity can provide, and respond to, the ever-shifting needs and desires of society.

As ecologists grapple with these great societal questions, they have developed ever larger datasets to test fundamental theory across systems \citep{Hampton2013}, but bigger data is messier data that ecologists are  not usually trained to handle. Such data generally requires a model of both the underlying biological processes and how the measurements were made. While some fields have long used these types of models \citep[generally in fields focused on inferring population sizes of things people want to eat or manage,][]{muthuku2008,zheng2007,trijoulet2018,strinella2020potential}, most have not. Thus ecologists have tried to adapt what they were trained in---traditional statistical methods (e.g., $F$ and $t$ tests) and a strong focus on null hypothesis testing. Often they have done this by fitting multi-way interaction terms (rather than using mechanistic models informed by biological understanding), using random effects to correct for group level factors (rather than explicit models of data-collection biases), or comparing across a large suite of models (because they cannot fit a single model that accounts for all the idiosyncrasies and biases).

The disconnect between the traditional statistical tools available to the ecologist and the fundamental theory they are trying to test is being made ever-clearer as ecologists are called upon to make forecasts. Bayesian approaches provide a pathway to powerful models that can transform how we understand our systems in terms of fundamental theory. Ecologists recognize this potential and are increasingly using Bayesian approaches, but are often inadequately prepared to notice or manage the pitfalls of such models. Many of these pitfalls can be avoided by approaching analyses through specific workflows \citep{betanworkflow,grinsztajn2021,vandeschoot2021}, which themselves are built on a process of how to do not just statistics, but how to do science \citep{box1976science}. % Maybe address workflow comment here? 

These approaches move away from a focus on null hypothesis testing, towards estimating effect sizes, using models calibrated (see Table \ref{tab:glossary}) and better understood through simulating data at multiple steps---
% WDP: The bits from "using models calibrated" aren't clear to me - maybe can you rephrase?
using a number of skills often reserved in ecology more for `theorists' than empirical ecologists.
% WDP: Here I think we're missing that nice link with fundamental theory, which I think will bring the 'previous generation' of scientists along with us for the ride here
Given the societal demands on ecologists today, we argue this divide is antiquated---especially as the average modern ecologist is computational, we believe they also need the skills to build and understand their own models. Breaking the training divide between theoretical and empirical ecologists, we argue, would yield greatly increased flexibility in what models can be fit and improved insights into the underlying systems.
% They move away from a focus on null hypothesis testing, towards estimating effect sizes, using models calibrated (see Table \ref{tab:glossary}) and better understood through simulating data at multiple steps---allowing greatly increased flexibility in what models can be fit and improved insights into the underlying systems. 
...
\section{A brief overview of Bayesian methods and their advantages}
...
\subsection{Bayesian and frequentist probability and inference}

Probability is often defined as ``the long-term frequency with which something happens.'' This is the frequentist definition, and it's also an intuitive definition, and it is excellent for checking whether a given observation (data, such as ``45 coins tossed out of 100 came up heads'') is consistent with a given model (such as ``50\% of tossed coins come up heads''). This excellence of probability of \emph{data} given \emph{model} ($p(data|model)$) leads to frequentist statistics focusing on \emph{refuting} whether a given set of observation is consistent with a particular null model, but makes it challenging to estimate the probability of a particular model ($p(model|data)$). Put another way, a frequentist test excells at telling us the probability that a given model is wrong (``I reject the null hypothesis that this is a fair coin''), but cannot tell us the probability that the coin is biased (``there is a 95\% probability that this coin comes up heads at least 80\% of the time'').

Bayesian methods can estimate the probability of a model, and that remarkable feat alone is enough to confer a major advantage when confronting a policy-maker (``what is the probability that those trees will die if I apply this herbicide''). This is done through the application of Bayes' theorem: 

\begin{equation}
  probability = p(model|data) = \frac{likelihood \cdot prior}{normaliser} = \frac{p(data|model) \cdot p(model)}{p(data)}
  \label{bayes_theorem}
\end{equation}

This equation, and its dependence on the \emph{normaliser} (a mathematical constant that makes sure our probability cannot go above 100\% or below 0\%; statisticians are lazy, and will not `give 110\%'), held back the practical use of Bayesian statistics for almost a century because that normaliser could rarely be analytically worked out. But one of the major advantages of Bayesian methods is that the solution to this problem---numerical simulations based on Markov Chain Monte Carlo (MCMC)---provide a huge additional advantage. Now that no analytical solution needs be found, any model that can be written out mathematically can be fit to data, giving the scientist more freedom of model structure.

\subsection{Flexibility} 
The oft-stated claim that ``you can fit any model you want'' with Bayesian methods is not entirely true \citep{BDA,reid2019}, but options can feel limitless compared to options available in popular modeling packages (\emph{e.g.}, \textsf{lme4}). As long as you can write out the likelihood of your desired model \citep[and sometimes even if you can't;][]{Sunnaaker2013} and assign priors to all parameters, you can generally fit it. This includes non-linear models, non-Gaussian families (e.g., Poisson, beta, or combinations thereof, such as hurdle models), hierarchical designs, `joint' models where parameters estimated in one equation appear in another propagating uncertainty, and many combinations of the above. Such flexibility is incredibly powerful in ecology where data are often influenced by complex spatial or temporal patterns, non-linear processes are widespread, and common data types are non-Gaussian (e.g. counts and percentage cover). 

Fitting a bespoke model to data also yields numbers we often really want but don't have access to in other approaches. We perform experiments because we want to know how a treatment affects something of interest---how much faster do plants grow with more warmth---but we often become more focused on whether the treatment was `significant' or not. We lose track of whether plants grew 5\% or 50\% more, and how consistent the effect was across individuals. But models can be designed to estimate and report effects per \degree C of warming with estimated uncertainty. While replication crises in other fields, driven in part by a overly zealous focus on $p$-values \citep{halsey2015,loken2017}, and the rise of meta-analyses in ecology \citep{Hampton2013} have led to a somewhat greater focus on `effect sizes' in ecology (often used to refer to very specific unitless statistics, such as Cohen's $d$, that can be difficult to connect to useful biological values), bespoke models take this to a new level.
% Researchers can estimate comparable effect sizes from $z$-scored data (in units of standard deviation), alongside estimates in meaningful natural units, such as per \degree C of warming or per hectare of habitat lost.
% WDP: Removed the above because you can do that in frequentist models too
...
\fi
% WDP: Again, Removed the above because I think we need the bridging section and we're over space. I think the 'not unique to Bayes' is perhaps worth pulling into the section above, but in a place that would be appropriate (or with the bit where you say "look this workflow could work for frequentist methods too")
...
% WDP: See my major comments. Broadly I think this is good, but I think the steps need to highlight why they're *exciting* and *solve a problem*. The calibration one just needs a bit more explicit linking to the text above, and as I look now is essentially almost there anyway. 
...
\iffalse
Our workflow is explained mostly program-agnostically. Though at times we assume a user of \textsf{Stan}, a relatively new probabalistic programming language, that interfaces with \textsf{R, Python, Julia} (and more) to write bespoke Bayesian models \citep{Carpenter:2017stan}. We focus on \textsf{Stan} as its MCMC algorithm (a variant of Hamiltonian Monte Carlo, HMC) is well-used, fast, and produces specific output to warn of model fit issues
% WDP: divergent transitions are not a feature of other MCMC methods so the statement I've deleted, combined with the "i.e.", comes across as misleading.
, but the basic workflow should apply to diverse implementations of Bayesian modeling, and can be extended to other approaches (frequentist, resampling, etc.). \\ % to write bespoke Bayesian models and underpins the \textsf{R} packages \textsf{brms} and \textsf{rstanarm}, which fit a suite of specific (pre-defined) models
\fi
% WDP: I re-read this bit and, given we're over space, I feel like we don't need to write it. I think we're name-dropping Julia to sound cool and, unless there is a strategic reason to do that, let's not do it
...
% WDP: I think what this section is missing is a conceptual diagram that shows how you might approach a problem as a 'traditional' undergrad ecologist (e.g., fitting a straight line through something), a 'traditional' PhD ecologist (e.g., fitting a curve line that has an aysmptote), and then an 'our workflow' ecologist (e.g., fitting an explicit mechanistic relationship). Then a lot of the 'merging theory with empirical data' stuff will shine through.


% Challenges of the workflow compared to how we traditionally train ecologists
